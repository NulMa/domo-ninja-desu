...
namespace DomoNinja.Core.Events
{
    ...
    public enum EventKind
    {
        ...
        /// <summary>
        /// 액티브 스킬을 알린다. Actor = 시전자, Target = -1, Value = 0(전투 시작 알림) 또는 1(실제 발동), Aux = 0.
        /// </summary>
        /// <remarks>
        /// ★ <b>스킬 <u>이름</u>은 싣지 않는다.</b> 이벤트는 참조 타입을 담지 않고(<see cref="GameEvent"/>),
        /// 한 유닛의 액티브 스킬은 전투 내내 하나로 고정이므로 <b>"누가"만 알면 화면이 이름을 안다</b> —
        /// 화면은 이미 로스터에서 <c>캐릭터 → 액티브 스킬</c> 을 읽고 있다.
        /// <para>
        /// ★ <b><see cref="Value"/> 를 둘로 나눈 이유.</b> 액티브 12종 중 <b>발동 순간이 있는 건 7종뿐</b>이다.
        /// 저격·난사·각인·주술·가호는 스탯·사거리·시작 버프라 전투 중 "터지는" 시점이 없어서,
        /// 발동만 알리면 <b>그 5종을 고른 플레이어는 스킬 이름을 한 번도 못 본다.</b>
        /// 그래서 전투 시작에 한 번 알리고(0), 실제로 터질 때 다시 알린다(1).
        /// </para>
        /// <para>
        /// 어느 쪽을 어떻게 보여줄지는 <b>화면이 정한다</b> — core 는 "알릴 만한 일이 있었다" 까지만 말한다.
        /// 모르는 항목을 만난 재생기는 그냥 흘리므로(<c>switch</c> 에 case 가 없으면 무시)
        /// <see cref="Dodge"/> 때와 마찬가지로 번호는 하나도 안 밀린다.
        /// </para>
        /// </remarks>
        SkillCast = 12,
    }
